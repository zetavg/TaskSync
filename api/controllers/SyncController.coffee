...
module.exports =
  ...
  sync: (req, res) ->
    syncLog = { remoteSyncs: {}, startTime: new Date() }
          ...
          serviceSync ->
            writeBack ->
              Task.destroy({deletedAt: {'!': null} }).exec (error, data) ->
                ...
                res.send(JSON.stringify(syncLog))
      else
        sails.log.error error
        syncLog.status = 'error'
        syncLog.error = error
        res.status(500).send(JSON.stringify(syncLog))
                  ...
                  asana.getTasks list.syncOptions, (error, data) ->
                    if error
                      ...
                      aTasks.forEach (aTask, index) ->
                        sails.log.info "preparing task for sync: #{aTask.id} (#{aTask.name})" if sails.logExtInfo
